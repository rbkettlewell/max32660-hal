...
pub trait AltFxn {
    fn mode_setup(block: &mut gpio::RegisterBlock, mask: u32);
}

impl AltFxn for Gpio {
    fn mode_setup(block: &mut gpio::RegisterBlock, mask: u32) {
        unsafe {
            block.en_set.write(|w| w.bits(mask));
            block.en1_clr.write(|w| w.bits(mask));
            block.en2_clr.write(|w| w.bits(mask));
        }
    }
}
impl AltFxn for AF1 {
    fn mode_setup(block: &mut gpio::RegisterBlock, mask: u32) {
        unsafe {
            block.en_clr.write(|w| w.bits(mask));
            block.en1_clr.write(|w| w.bits(mask));
            block.en2_clr.write(|w| w.bits(mask));
        }
    }
...
impl AltFxn for AF2 {
    fn mode_setup(block: &mut gpio::RegisterBlock, mask: u32) {
        unsafe {
            block.en_clr.write(|w| w.bits(mask));
            block.en1_set.write(|w| w.bits(mask));
            block.en2_clr.write(|w| w.bits(mask));
        }
    }
}
...
// ===============================================================
/// Generic $PX pin
pub struct Pin<AF: AltFxn, IO> {
    pin: u8,
    _af: PhantomData<AF>,
    _io: PhantomData<IO>,
}

impl<AF: AltFxn, IO> Pin<AF, IO> {
    ...
    fn block(&self) -> &mut gpio::RegisterBlock {
        let ptr = unsafe { &mut *(P0::ptr() as *mut _) };
        ptr
    }

    fn into_mode<A: AltFxn, I>(mut self) -> Self {
        A::mode_setup(&mut self.block(), self.mask());
        Self::new(self.pin)
    }
...
impl <AF: AltFxn, MODE> InputPin for Pin <AF, Input<MODE>> {
    type Error = Void;
...
impl <AF: AltFxn, MODE> OutputPin for Pin <AF, Output<MODE>> {
    type Error = Void;
...
impl <AF: AltFxn, MODE> StatefulOutputPin for Pin<AF, Output<MODE>> {
...
impl <AF: AltFxn, MODE> Pin<AF, Output<MODE>> {
...
macro_rules! gpio {
    (
        ...
        pub mod $px {
            ...
            // ===============================================================
            $(
                pub struct $PXi<AF: AltFxn, IO> {
                    _af: PhantomData <AF>,
                    _io: PhantomData <IO>,
                }

                impl<AF: AltFxn, IO> $PXi<AF, IO> {
                ...
                impl<AF: AltFxn, IO> InputPin for $PXi<AF, Input<IO>> {
                    type Error = Void;
                ...
                impl<AF: AltFxn, IO> From<$PXi<AF, IO>> for Pin<AF, IO> {
                    fn from(value: $PXi<AF, IO>) -> Self {
                        value.degrade()
                    }
                }

                impl<AF: AltFxn, IO> OutputPin for $PXi<AF, Output<IO>> {
                    type Error = Void;
                ...
                impl <AF: AltFxn, IO> $PXi <AF, Output<IO>> {
                    pub fn set_drive_strength(&self, drive_strength: DriveStrength) -> () {
                        let ds_settings = drive_strength.get_setting();
                        let ds_val = ds_settings.ds as u32;
                        let ds1_val = ds_settings.ds1 as u32;
                ...
                impl<AF: AltFxn, IO> StatefulOutputPin for $PXi<AF, Output<IO>> {
                    /// Is the output pin set as high?
                    fn is_set_high(&self) -> Result<bool, Self::Error> {
                        self.is_set_low().map(|v| !v)
                    }
